...
final class InputTypeTests: XCTestCase {
    ...
    func testArrayInputWithMessage() throws {
        ...
        case let .array(items):
            ...
            if case let .message(message) = items[0] {
    ...
    func testMessageWithArrayContent() throws {
        ...
        if case let .message(message) = item {
            XCTAssertEqual(message.role, "user")
            if case let .array(contentItems) = message.content {
                XCTAssertEqual(contentItems.count, 2)

                // Check text content
                if case let .text(textContent) = contentItems[0] {
                ...
                // Check image content
                if case let .image(imageContent) = contentItems[1] {
    ...
    func testInputFileContent() throws {
        ...
        if case let .file(fileContent) = content {
    ...
    func testOutputTextContent() throws {
        ...
        if case let .outputText(outputContent) = content {
    ...
    func testRefusalContent() throws {
        ...
        if case let .refusal(refusalContent) = content {
    ...
    func testFunctionToolCall() throws {
        ...
        if case let .functionToolCall(call) = item {
    ...
    func testFunctionToolCallOutput() throws {
        ...
        if case let .functionToolCallOutput(output) = item {
    ...
    func testComplexConversation() throws {
        ...
        case let .array(items):
            ...
            if case let .message(systemMsg) = items[0] {
            ...
            // Verify user message
            if case let .message(userMsg) = items[1] {
            ...
            if case let .functionToolCall(call) = items[2] {
                XCTAssertEqual(call.name, "get_weather")
            } else {

    ...
    public init(from decoder: Decoder) throws {
        ...
        case "input_file":
            if try container.decodeIfPresent(String.self, forKey: .fileId) != nil {
                self = .fileId(try singleValueContainer.decode(FileIdContent.self))
            } else {
                self = .fileData(try singleValueContainer.decode(FileDataContent.self))
            }
        case "input_audio":
            self = .audio(try singleValueContainer.decode(AudioContent.self))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .type,
                in: container,

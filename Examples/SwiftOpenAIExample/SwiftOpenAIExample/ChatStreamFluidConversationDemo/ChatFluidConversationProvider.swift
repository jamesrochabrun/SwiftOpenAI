...
@Observable class ChatFluidConversationProvider {
   ...
   private func updateParameterMessagesArray(_ message: ChatCompletionParameters.Message) {
      ...
      print("NEWCOUNT \(parameterMessages.count) message \(message)")
      if parameterMessages.count > Self.parameterMessagesMaxStorageCount {
         print("NEWCOUNT \(parameterMessages.count) removed message \(parameterMessages[0])")

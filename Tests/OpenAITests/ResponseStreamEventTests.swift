...
final class ResponseStreamEventTests: XCTestCase {
    ...
    func testResponseCreatedEvent() throws {
        ...
        if case let .responseCreated(createdEvent) = event {
    ...
    func testResponseCompletedEvent() throws {
        ...
        if case let .responseCompleted(completedEvent) = event {
    ...
    func testOutputItemAddedEvent() throws {
        ...
        if case let .outputItemAdded(addedEvent) = event {
            ...
            if case let .message(message) = addedEvent.item {
    ...
    func testOutputItemAddedWithFunctionCall() throws {
        ...
        if case let .outputItemAdded(addedEvent) = event {
            if case let .functionCall(functionCall) = addedEvent.item {
    ...
    func testOutputItemAddedWithImageGeneration() throws {
        ...
        if case let .outputItemAdded(addedEvent) = event {
            if case let .imageGenerationCall(imageGen) = addedEvent.item {
    ...
    func testOutputItemAddedWithCodeInterpreter() throws {
        ...
        if case let .outputItemAdded(addedEvent) = event {
            if case let .codeInterpreterCall(codeInterpreter) = addedEvent.item {
    ...
    func testOutputItemDoneWithMCPCall() throws {
        ...
        if case let .outputItemDone(doneEvent) = event {
            if case let .mcpCall(mcpCall) = doneEvent.item {
    ...
    func testOutputTextDeltaEvent() throws {
        ...
        if case let .outputTextDelta(deltaEvent) = event {
    ...
    func testOutputTextDoneEvent() throws {
        ...
        if case let .outputTextDone(doneEvent) = event {
    ...
    func testFunctionCallArgumentsDeltaEvent() throws {
        ...
        if case let .functionCallArgumentsDelta(deltaEvent) = event {
    ...
    func testFunctionCallArgumentsDoneEvent() throws {
        ...
        if case let .functionCallArgumentsDone(doneEvent) = event {
    ...
    func testFileSearchCallEvents() throws {
        ...
        if case let .fileSearchCallInProgress(event) = inProgressEvent {
        ...
        if case let .fileSearchCallSearching(event) = searchingEvent {
        ...
        if case let .fileSearchCallCompleted(event) = completedEvent {
    ...
    func testWebSearchCallEvents() throws {
        ...
        for (eventType, expectedCase) in eventTypes {
            ...
            switch event {
            case let .webSearchCallInProgress(e) where expectedCase == "webSearchCallInProgress":
                XCTAssertEqual(e.type, eventType)
                XCTAssertEqual(e.itemId, "ws_123")

            case let .webSearchCallSearching(e) where expectedCase == "webSearchCallSearching":
                XCTAssertEqual(e.type, eventType)

            case let .webSearchCallCompleted(e) where expectedCase == "webSearchCallCompleted":
                XCTAssertEqual(e.type, eventType)
    ...
    func testReasoningSummaryTextDeltaEvent() throws {
        ...
        if case let .reasoningSummaryTextDelta(deltaEvent) = event {
    ...
    func testImageGenerationEvents() throws {
        ...
        if case let .imageGenerationCallPartialImage(event) = partialImageEvent {
    ...
    func testOutputItemLocalShellCall() throws {
        ...
        if case let .outputItemAdded(addedEvent) = event {
            if case let .localShellCall(shellCall) = addedEvent.item {
    ...
    func testOutputItemMCPListTools() throws {
        ...
        if case let .outputItemDone(doneEvent) = event {
            if case let .mcpListTools(listTools) = doneEvent.item {
    ...
    func testOutputItemMCPApprovalRequest() throws {
        ...
        if case let .outputItemAdded(addedEvent) = event {
            if case let .mcpApprovalRequest(approval) = addedEvent.item {

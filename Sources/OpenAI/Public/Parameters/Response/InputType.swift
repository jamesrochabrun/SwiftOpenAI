...
public enum InputItem: Codable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case let .outputMessage(message):
            try message.encode(to: encoder)
        case let .fileSearchToolCall(call):
            try call.encode(to: encoder)
        case let .functionToolCall(call):
            try call.encode(to: encoder)
        case let .functionToolCallOutput(output):
            try output.encode(to: encoder)
        case let .computerToolCallOutput(output):
            try output.encode(to: encoder)
        case let .localShellCallOutput(output):
            try output.encode(to: encoder)
...
public struct ComputerToolCallOutput: Codable {
    public init(callId: String, id: String, output: ComputerScreenshot, acknowledgedSafetyChecks: [SafetyCheck]? = nil, status: String? = nil) {
        self.callId = callId
        self.id = id
        self.output = output

...
class ResponseStreamProvider {
      ...
      for message in messages.dropLast(2) { // Exclude current user message and streaming placeholder
        ...
        case .assistant:
          if let responseId = message.responseId {
            let outputContent = OutputTextContent(
              text: content
            )
            let contentItem = ContentItem.outputText(outputContent)
            inputArray.append(.outputMessage(OutputMessage(
              content: [contentItem],
              id: responseId,
              role: "assistant",
              status: "completed"
  ...
  private func updateStreamingMessage(with content: String) {
    guard let messageId = currentStreamingMessage?.id,

...
public enum Tool: Codable {
   ...
   public enum FileSearchFilter: Codable {
      ...
      public init(from decoder: Decoder) throws {
         ...
         if let type = try? container.decode(String.self, forKey: .type) {
            if type == "and" || type == "or" {
               self = .compound(try decoder.singleValueContainer().decode(CompoundFilter.self))
            } else {
               self = .comparison(try decoder.singleValueContainer().decode(ComparisonFilter.self))
            }
         } else {
   ...
   public struct UserLocation: Codable {
      ...
      public init(from decoder: Decoder) throws {
         ...
         if let decodedType = decodedType, decodedType != "approximate" {
   ...
   public init(from decoder: Decoder) throws {
      ...
      switch type {
      case "file_search":
         self = .fileSearch(try singleValueContainer.decode(FileSearchTool.self))
      case "function":
         self = .function(try singleValueContainer.decode(FunctionTool.self))
      case "computer_use_preview":
         self = .computerUse(try singleValueContainer.decode(ComputerUseTool.self))
      case "web_search_preview", "web_search_preview_2025_03_11":
         self = .webSearch(try singleValueContainer.decode(WebSearchTool.self))
      default:
         throw DecodingError.dataCorruptedError(
            forKey: .type,
            in: container,
